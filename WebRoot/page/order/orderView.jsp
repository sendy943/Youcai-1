    ...
    <td><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
      ...
      <table width="100%" border="0" align="center" cellpadding="3" cellspacing="2">
        <tr>
          ...
          <td colspan="2" bgcolor="#FFFFFF">${order.paymentWay.name }  <c:if test="${order.state!='RECEIVED' && order.state!='CANCEL'}"><a href="/core/page/order/updatePaymentWay.jsp?orderId=${order.orderId }&paymentWay=${order.paymentWay }&deliverWay=${acceptor.deliverWay }">修改</a></c:if></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="2" bgcolor="#FFFFFF">${acceptor.deliverWay.name }  <c:if test="${order.state!='RECEIVED' && order.state!='CANCEL'}"><a href="/core/page/order/updateDeliverWay.jsp?orderId=${order.orderId }&deliverWay=${acceptor.deliverWay }&paymentWay=${order.paymentWay }">修改</a></c:if></td>
        </tr>
        <tr>
...
<c:forEach items="${order.items}" var="item">
        <tr>
          ...
          <td align="center" bgcolor="#FFFFFF">${item.amount } <c:if test="${order.state!='RECEIVED' && order.state!='CANCEL'}"><a href="/core/page/order/updateProductAmount.jsp?oitemId=${item.itemId }&orderId=${order.orderId }&amount=${item.amount }">修改</a></c:if></td>
        ...
        <tr>
          <td colspan="7" align="right" bgcolor="#FFFFFF"><p>商品合计：￥${order.productTotalPrice }元&nbsp;&nbsp;配送费：￥${order.deliverFee }元 <c:if test="${order.state!='RECEIVED' && order.state!='CANCEL'}"><a href="/core/page/order/updateDeliverFee.jsp?orderId=${order.orderId}&deliverFee=${order.deliverFee }">修改</a></c:if>&nbsp;&nbsp;订单合计：￥${order.totalPrice }元<br />
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="2">
  <tr>
    ...
    <input type="button" class="frm_btn" value="打印订单" onclick="JavaScript:winOpen('/core/control/order/showPrintUI?order.orderId=${order.orderId }','打印',700,450)"/>&nbsp;
	<c:if test="${order.state!='RECEIVED' && order.state!='CANCEL'}">
	<input type="button" class="frm_btn" value="解锁退出" onclick="JavaScript:window.location.href='/core/control/order/unlockOrder?order.orderId=${order.orderId }'"/>
	</c:if>
	</td>
  </tr>
</table>
...
<table width="90%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr>
    <td colspan="2"  bgcolor="6f8ac4"><FONT COLOR="#FFFFFF">客服留言</FONT> &nbsp; <input type="button" class="frm_btn" value="客服留言" onclick="JavaScript:window.location.href='/core/control/order/showAddMessageUI?orderId=${order.orderId}'"/></td>
  </tr>
  <tr>
